...
namespace asgn5v1
{
    ...
    public class Transformer : System.Windows.Forms.Form
    {
        ...
        public Transformer()
        {
            ...
            Text = "COMP 4560:  Assignment 5 (A00841883) (Kevin Tam)";
            ResizeRedraw = true;
        ...
        void start()
        {
            ...
            double returnZeroX = vertices[0, 0];
            double returnZeroY = vertices[0, 1];

            double scaleWindow = y / (2 * returnZeroY);

            //Translate the middle point to 0,0 
            tnet = multiplyMatrix(translate(-returnZeroX, -returnZeroY, 0), scale(scaleWindow, scaleWindow, scaleWindow));
            tnet = multiplyMatrix(tnet, reflect("xz"));
            tnet = multiplyMatrix(tnet, translate(x, y, 0));
            this.ctrans = tnet;
        ...
        double[,] scale(double scalarX, double scalarY, double scalarZ)
        {
            double[,] matrix = new double[4, 4];
            setIdentity(matrix, 4, 4);
        ...
        {
            timer.Interval = 5;
            timer.Tick += new EventHandler(keepRotating);
            timer.Stop();
            if (e.Button == transleftbtn)
            {
                this.ctrans = multiplyMatrix(this.ctrans, translate(-75, 0, 0));
                Refresh();
            }
            ...
            {
                this.ctrans = multiplyMatrix(this.ctrans, translate(0, -75, 0));
                Refresh();
            }
            ...
            {
                this.ctrans = multiplyMatrix(this.ctrans, translate(-scrnpts[0, 0], -scrnpts[0, 1], -scrnpts[0, 2]));
                this.ctrans = multiplyMatrix(this.ctrans, scale(1.10, 1.10, 1.10));
                this.ctrans = multiplyMatrix(this.ctrans, translate(scrnpts[0, 0], scrnpts[0, 1], scrnpts[0, 2]));
            ...
            {
                this.ctrans = multiplyMatrix(this.ctrans, translate(-scrnpts[0, 0], -scrnpts[0, 1], -scrnpts[0, 2]));
                this.ctrans = multiplyMatrix(this.ctrans, scale(0.90, 0.90, 0.90));
                this.ctrans = multiplyMatrix(this.ctrans, translate(scrnpts[0, 0], scrnpts[0, 1], scrnpts[0, 2]));
            ...
            {
                this.ctrans = multiplyMatrix(this.ctrans, translate(-scrnpts[0, 0], -scrnpts[0, 1], -scrnpts[0, 2]));
                this.ctrans = multiplyMatrix(this.ctrans, rotate('x'));
                this.ctrans = multiplyMatrix(this.ctrans, translate(scrnpts[0, 0], scrnpts[0, 1], scrnpts[0, 2]));
                Refresh();
            ...
            {
                this.ctrans = multiplyMatrix(this.ctrans, translate(-scrnpts[0, 0], -scrnpts[0, 1], -scrnpts[0, 2]));
                this.ctrans = multiplyMatrix(this.ctrans, rotate('y'));
                this.ctrans = multiplyMatrix(this.ctrans, translate(scrnpts[0, 0], scrnpts[0, 1], scrnpts[0, 2]));
                Refresh();
            }
            ...
            {
                this.ctrans = multiplyMatrix(this.ctrans, translate(-scrnpts[0, 0], -scrnpts[0, 1], -scrnpts[0, 2]));
                this.ctrans = multiplyMatrix(this.ctrans, rotate('z'));
                this.ctrans = multiplyMatrix(this.ctrans, translate(scrnpts[0, 0], scrnpts[0, 1], scrnpts[0, 2]));
                Refresh();
            }
            ...
            {
                double copyMatrix = ctrans[3, 1];
                ctrans[3, 1] = 0.0d;
                ctrans = multiplyMatrix(ctrans, shear("right"));
                ctrans[3, 1] = copyMatrix;
                Refresh();
            }
        ...
        private void keepRotating(object sender, EventArgs e)
        {
